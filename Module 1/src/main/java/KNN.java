...
public class KNN {
    ...
    public static boolean predictSuccess(Student [] students, double[] grades, int k) {
        ...
        // My algorithm is not the best... did what i could
            //1. calculate euclidian distance for each students in dataset
            //2. Store the euclidian distance and sucess (converted to binairy) in a nx2 matrix named myTable: [[eucdist,succes][...,...]]
            //3. Sort myTable from euclidian distance
            //4. add to "pos" variable : the num of sucess in k first (so closest to evaluated) (which is just the value 0 or 1 in the myTable)
            //5. if pos > k/2 return true

        double[][] myTable = new double[students.length][2];
        double res = 0;
        double pos = 0;
        ...
        for (int i = 0; i < students.length; i++) {
            ...
            myTable[i][0] =  euclideanDistance(students[i].grades,grades);
            myTable[i][1] = res;
        }
        Arrays.sort(myTable, Comparator.comparingDouble(a -> a[0]));
        for (int i=0; i<k; i++){
            pos += myTable[i][1];
        }
        if (pos > k/2){
