...
public class FuturesLibrary {
    ...
    public static int countMatchingBooks(Library library,
        ...
        for (int i = startBookIndex; i<startBookIndex + countBooks; i++){
            if (predicate.test(library.getBook(i))){
                count += 1;
            }
        }
         return count;
    ...
    public static int countMatchingBooksWithThreads(Library library,
        ...
        int sizeperthread = size/countThreads;
        System.out.println("-----------------------------------------------------------" );

        System.out.println("Size per thread : " + sizeperthread);
        System.out.println("Count tread : " + countThreads);
        System.out.println("Size total : " + size);

        int counttotal=0;


        Stack<Future<Integer>> sta = new Stack<>();
        ...
        for (int i = 0; i < countThreads; i++){
            ...
            if (i == countThreads-1){
                debut = size-sizeperthread;//not sure minus 1
            }
            else {
                debut = i * sizeperthread;

            }
            System.out.println("Début : " + debut);
            sta.add(executor.submit(()->countMatchingBooks(library,predicate,debut,sizeperthread)));
