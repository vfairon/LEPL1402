...
public class GlobalWarming {
    ...
    static public class IPCCDatabase {
        ArrayList<RecordObserver> obs = new ArrayList<>();
        ...
        public void temperatureMeasured(String place, float temperature) {
            if (!temp.containsKey(place)){
                for (RecordObserver ob : obs){
                    ob.signalNewRecord(place, temperature);
                }
                temp.put(place,temperature);
            }
            else {
                if (temp.get(place) < temperature){
                    for (RecordObserver ob : obs){
                        ob.signalNewRecord(place, temperature);
                    }
                    temp.replace(place,temperature);
